...
Object.extend(String.prototype, (function() {
    ...
    function escapeHTML() {
        var quotes = arguments.length > 0 ? arguments[0] : true;
    ...
    function unescapeHTML() {
        var quotes = arguments.length > 0 ? arguments[0] : true;
        var s = this.stripTags().replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&amp;/g, '&');
        if (quotes)
            s = s.replace(/&quot;/g, '"').replace(/&#039;/g, "'");
        return s
    }
...
Element.Methods = {visible: function(element) {
    ...
    for (var property in styles)
        if (property == 'opacity')
            ...
            elementStyle[(property == 'float' || property == 'cssFloat') ? (Object.isUndefined(elementStyle.styleFloat) ? 'cssFloat' : 'styleFloat') : property.camelize()] = styles[property]
        }
    return element
...
if ('getBoundingClientRect' in document.documentElement) {
    Object.extend(Element.Methods, {viewportOffset: function(forElement) {
        var element = forElement;
        var rect = element.getBoundingClientRect();
        var docEl = document.documentElement;
        return Element._returnOffset(rect.left - docEl.clientLeft, rect.top - docEl.clientTop)
    }})
}
...
},descendant: 'c = "descendant";',child: 'c = "child";',adjacent: 'c = "adjacent";',laterSibling: 'c = "laterSibling";'},patterns: [{name: 'laterSibling',re: /^\s*~\s*/}, {name: 'child',re: /^\s*>\s*/}, {name: 'adjacent',re: /^\s*\+\s*/}, {name: 'descendant',re: /^\s/}, {name: 'tagName',re: /^\s*(\*|[\w\-]+)(\b|$)?/}, {name: 'id',re: /^#([\w\-\*]+)(\b|$)/}, {name: 'className',re: /^\.([\w\-\*]+)(\b|$)/}, {name: 'pseudo',re: /^:((first|last|nth|nth-last|only)(-child|-of-type)|empty|checked|(en|dis)abled|not)(\((.*?)\))?(\b|$|(?=\s|[:+~>]))/}, {name: 'attrPresence',re: /^\[((?:[\w-]+:)?[\w-]+)\]/}, {name: 'attr',re: /\[((?:[\w-]*:)?[\w-]+)\s*(?:([!^$*~|]?=)\s*((['"])([^\4]*?)\4|([^'"][^\]]*?)))?\]/}],assertions: {tagName: function(element, matches) {
...
if (!window.NovaOptions)
    window.NovaOptions = {};
if (!window.Nova)
    window.Nova = {};
Nova.initializing = true;
Nova.version = '$Rev: 1021 $';
if (typeof (Nova.img_lazyload) == "undefined")
    Nova.img_lazyload = false;
Nova.page_loaded = false;
Nova.optionDefault = function(n, v) {
    if (NovaOptions[n] === undefined)
        NovaOptions[n] = v
};
...
function nova_apply(func, parentElement) {
    ...
    for (var i = 0; i < children.length; i++) {
        ...
        if (ar != null)
            func(Element.extend(children[i]), ar[0].replace(/^\s*|\s*$/g, ""))
    }
}
function nova_call(url, param, callback, id, remote) {
    var loader = new NovaRequestLoader("", url, param, "post", callback, remote);
    if (remote == undefined) {
        loader.start()
    }
}
function nova_request(callback, url, param, method) {
    var loader = new NovaRequestLoader("", url, param, method, callback, undefined, true);
    loader.start()
}
function nova_update(id, url, param, method, callback) {
    var loader = new NovaRequestLoader(id, url, param, method, callback, undefined, true);
    loader.start()
}
function nova_update_fade(id, url, param, method, callback) {
    nova_update(id, url, param, method, callback, undefined, true)
}
...
Object.extend(Object.extend(NovaRequestLoader.prototype), {initialize: function(id, url, param, method, callback, remote, rawParam) {
    ...
    if (remote != undefined) {
        ...
        if (rawParam) {
            ...
            this.options.parameters = {"__ap": Object.toJSON(param)}
        }
    ...
    if (window.nova_error_hook)
        window.nova_error_hook();
    else
        Nova.log('Error in transport.')
},recv: function(transport) {
    if ($(this.id)) {
        $(this.id).update(transport.responseText)
    }
    ;
    var obj;
    try {
...
Nova.addInitFunc = function(listener) {
    if (!Nova.initializing)
        listener();
    else
        Nova.initCallbacks[Nova.initCallbacks.length] = listener
};
...
Nova.Cookie = {set: function(name, value, days, path, domain) {
    if (days) {
        var date = new Date();
...
Nova.waiting = function(name) {
    ...
    if (typeof window[name + '_handler'] == 'function')
        window[name + '_handler']();
    else
        eval(window[name + '_handler']);
...
function getParamsFromUrl(url) {
    ...
    if (index != -1 && url) {
        var paramsArray = url.substring(index + 1).split("&");
...
(function() {
    var LazierLoadImage = Class.create();
    LazierLoadImage.prototype = {element: null,position: null,lazyScroller: null,initialize: function(image) {
        this.element = image;
        this.imageSrc = this.element.readAttribute("_src");
        this.position = Position.page(this.element);
        var windowHeight = document.viewport.getHeight() || document.documentElement.clientHeight;
        var scrollOffsets = document.viewport.getScrollOffsets();
        var url = window.location.href;
        var params = getParamsFromUrl(url);
        var isLoadImg = false;
        if (params["lazyloadimg"] && params["lazyloadimg"] == 0) {
            isLoadImg = true
        }
        if (windowHeight === undefined || scrollOffsets[1] === undefined || this.position[1] === undefined || isLoadImg) {
            this.loadImage();
            return false
        }
        if (this.position[1] < (scrollOffsets[1] + windowHeight * 2)) {
            this.loadImage()
        } else {
            this.lazyScroller = this.lazyScroll.bindAsEventListener(this);
            Event.observe(window, 'resize', this.lazyScroller, false);
            Event.observe(window, 'scroll', this.lazyScroller, false)
        }
    },lazyScroll: function() {
        var windowHeight = document.viewport.getHeight() || document.documentElement.clientHeight;
        var scrollOffsets = document.viewport.getScrollOffsets();
        if (windowHeight === undefined || scrollOffsets[1] === undefined || this.position[1] === undefined) {
            return false
        }
        if (this.position[1] < (scrollOffsets[1] + windowHeight * 2)) {
            this.loadImage();
            Event.stopObserving(window, 'scroll', this.lazyScroller);
            Event.stopObserving(window, 'resize', this.lazyScroller)
        }
    },loadImage: function() {
        var image = new Image();
    ...
    window.nova_init_hook_delayload = function() {
        if (Nova.img_lazyload == false) {
            return
        }
        var imgs = $$("img[_src]");
        imgs.each(function(img) {
            var container = img.parentNode;
            var isBindLazyLoad = true;
            while (container && container.nodeType == 1) {
                var notLoadImg = Element.readAttribute(container, "notLoadImg");
                if (notLoadImg && notLoadImg == "true") {
                    isBindLazyLoad = false;
                    break
                }
                container = container.parentNode
            }
            if (isBindLazyLoad) {
                new LazierLoadImage(img)
            }
        })
    }
})();
Object.extend(String.prototype, {evalScripts: function() {
    return this.extractScripts().map(function(script) {
        return eval(script)
    })
}});
